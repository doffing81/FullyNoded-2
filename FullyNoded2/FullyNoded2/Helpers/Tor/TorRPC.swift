...
class TorRPC {
    ...
    func executeRPCCommand(walletName: String, method: BTC_CLI_COMMAND, param: Any, completion: @escaping (Result<Any?, TorRPCError>) -> () = {_ in}) {
        ...
        do {
            ...
            queue.async {
                ...
                let task = TorClient.sharedInstance.session.dataTask(with: request as URLRequest) { [weak self] (data, response, error) in
                    if error != nil {
                        ...
                        if let urlContent = data {
                            do {
                                ...
                                if let errorCheck = jsonAddressResult["error"] as? NSDictionary {
                                    ...
                                    let responseString = self?.checkCommandForResponse(command: method)
